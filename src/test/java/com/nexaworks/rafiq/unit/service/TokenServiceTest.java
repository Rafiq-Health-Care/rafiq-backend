...
public class TokenServiceTest {
  ...
  @InjectMocks @Spy TokenServiceImpl tokenService;
  ...
  void generateAccessToken_ShouldThrowException_WhenUserIsNull() {
    assertThrows(
        UserNotFoundException.class, () -> tokenService.generateAccessToken(Optional.empty()));
    verify(tokenRepository, never()).save(any(Token.class));
  }
  ...
  void verifyOtp_ShouldThrowException_WhenNoOtpTokenMatch() {
    ...
    assertThrows(
        TokenNotFoundException.class, () -> tokenService.verifyOtp("john@gmail.com", "1234"));
  ...
  void verifyOtp_shouldThrowException_WhenOtpTokenIsExpired() {
    ...
    when(tokenRepository.findByToken(anyString())).thenReturn(Optional.of(token));
    assertThrows(
        TokenInvalidException.class, () -> tokenService.verifyOtp(user.getEmail(), "123456"));
    verify(tokenRepository, times(1)).findByToken(anyString());
  }
  ...
  void verifyOtp_shouldThrowException_WhenEmailDoesNotMatch() {
    ...
    when(tokenRepository.findByToken(anyString())).thenReturn(Optional.of(token));
    assertThrows(
        TokenInvalidException.class, () -> tokenService.verifyOtp("elbialy@gmail.com", "123456"));
    verify(tokenRepository, times(1)).findByToken(anyString());
  }
