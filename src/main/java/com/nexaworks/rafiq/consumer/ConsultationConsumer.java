...
public class ConsultationConsumer {
    ...
    public void handleExpiration(String consultationId, Message message, Channel channel,
        ...
        try {
            ...
            retryOrDlq(consultationId, message, channel, tag, CONSULTATION_EXPIRATION_EXCHANGE,
    ...
    public void handlePreparation(String consultationId, Message message, Channel channel,
        ...
        try {
            ...
            rabbitTemplate.convertAndSend(CONSULTATION_PREPARATION_EXCHANGE,
                    CONSULTATION_PREPARATION_DLQ_ROUTING_KEY, consultationId);
            channel.basicAck(tag, false);
        } catch (Exception e) {
            log.error("Failed to prepare consultation: {}", consultationId, e);
            retryOrDlq(consultationId, message, channel, tag, CONSULTATION_PREPARATION_EXCHANGE,
                    CONSULTATION_PREPARATION_RETRY_ROUTING_KEY,
                    CONSULTATION_PREPARATION_DLQ_ROUTING_KEY);
        }
    }

    private void retryOrDlq(String id, Message message, Channel channel, long tag, String exchange,
            String retryKey, String dlqKey) throws IOException {
        ...
        if (retryCount >= MAX_RETRIES) {
            ...
            rabbitTemplate.convertAndSend(exchange, retryKey, id, msg -> {
                msg.getMessageProperties().setHeader("x-retry-count", next);
                return msg;
            });
        }
        channel.basicAck(tag, false);
