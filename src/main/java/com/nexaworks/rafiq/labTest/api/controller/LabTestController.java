...
public class LabTestController {
    ...
    public ResponseEntity<Void> testResults(
            @RequestBody @Valid TestResultRequest testResultRequest, Authentication authentication) {

        labTestService.addTest(testResultRequest.testId(), testResultRequest.name(),
                testResultRequest.date(), resultMapper.toEntity(testResultRequest.tests()), (UUID) authentication.getPrincipal());
    ...
    public ResponseEntity<PageResponse<TestResponse>> getAllTests(
        ...
        return ResponseEntity.ok().body(
                pageMapper.mapToTestResponse(labTestService.getAll(page, size, sort, direction, (UUID) authentication.getPrincipal())));
    }

    @GetMapping("/{test-id}")
    public ResponseEntity<TestResultsResponse> getTest(@PathVariable("test-id") UUID testId,Authentication authentication) {
        return ResponseEntity.ok()
                .body(testMapper.mapToTestResponse(labTestService.getTest(testId, (UUID) authentication.getPrincipal())));
    }
    ...
    public ResponseEntity<Integer> deleteAllTests(Authentication authentication) {
        return ResponseEntity.ok().body(labTestService.deleteAll((UUID) authentication.getPrincipal()));
    }
    ...
    public ResponseEntity<Void> updateTest(@RequestBody @Valid TestResultRequest testResultRequest,
        ...
        labTestService.update(testId, testResultRequest,
                resultMapper.toEntity(testResultRequest.tests()), (UUID) authentication.getPrincipal());
        return ResponseEntity.ok().build();
    }
}
